...
function GetFileBOM($fileID)
{
	...
	$bom.InstArray | Where-Object { $_.ParId -eq 0 } | ForEach-Object { 
		...
		If ($comp.CompTyp -ne "Virtual"){
			...
			$dsWindow.FindName("btnExportCSV").Visibility = "Visible"
			$dsWindow.FindName("btnExportXML").Visibility = "Visible"
		}
		Else { 
			...
			$dsWindow.FindName("btnExportCSV").Visibility = "Collapsed"
			$dsWindow.FindName("btnExportXML").Visibility = "Collapsed"
		}
		#endregion
	}
